...
# -*- coding: utf-8 -*-
"""
train_csinet_baseline_8ue.py
=============================
CsiNet+ E2E Precoder Baseline 训练 (K=8)
对标 8UE × 128SC 的 Pure FDMA 模式 (固定预算 D_tot=128, 每用户 D_f=16)。

与 train_csinet_baseline.py (K=4) 的唯一区别: 导入 exp_8ue_128sc 配置。
模型代码 (models/csinet_plus_precoder.py) 完全复用 —— 它全程读 cfg, K-general。
ckp 自动落到 ckp/csinet_baseline_8ue_128sc_fdma16/, 与 K=4 隔离。

跟 Swin baseline 严格对齐:
  - 同样的 SNR curriculum
  - 同样的 SumRateLoss
  - 同样的 OneCycleLR + grad clip
  - 同样的 cache_data 策略
"""
...
class SumRateLoss(nn.Module):
    ...
    def forward(self, W, H_dl_true, noise_power):
        ...
        power = h_eff.abs() ** 2
        sig_pwr = torch.stack([power[:, i, i, :] for i in range(power.shape[1])],
                              dim=1)
...
def main():
    ...
    assert cfg.DIM_AIRCOMP_SHARED == 0, \
        "CsiNet+ baseline 只对标 Pure FDMA 模式, 请把 DIM_AIRCOMP_SHARED 设为 0"
    assert cfg.DIM_FDMA_PER_USER > 0, \
        "CsiNet+ baseline 需要 FDMA 反馈, 请设置 DIM_FDMA_PER_USER > 0"

    # 路径
    save_dir = (f"ckp/csinet_baseline_{cfg.K_USERS}ue_{cfg.SUBCARRIERS}sc"
                f"_fdma{cfg.DIM_FDMA_PER_USER}")
    os.makedirs(save_dir, exist_ok=True)
    BEST_PATH = os.path.join(save_dir, "csinet_best.pth")
    LAST_PATH = os.path.join(save_dir, "csinet_latest.pth")

    print(f"🎯 [CsiNet+ Baseline] {cfg.K_USERS}UE × {cfg.SUBCARRIERS}SC, "
    ...
    print(f"📐 [CsiNet+] Params: {n_params/1e6:.2f}M")
    ...
    pbar = tqdm(range(start_step, cfg.TOTAL_STEPS + 1),
                desc="CsiNet+-Baseline-8UE", dynamic_ncols=True)
