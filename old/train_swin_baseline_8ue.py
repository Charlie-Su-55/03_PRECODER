...
# -*- coding: utf-8 -*-
"""
train_swin_baseline_8ue.py
==========================
SwinCFNet + E2E ZF Baseline 训练 (K=8)
对标 8UE × 128SC 的 Pure FDMA 模式 (固定预算 D_tot=128, 每用户 D_f=16)。

与 train_swin_baseline.py (K=4) 的唯一区别: 导入 exp_8ue_128sc 配置。
模型代码 (models/swin_precoder.py) 完全复用 —— 它全程读 cfg, K-general。
ckp 自动落到 ckp/swin_baseline_8ue_128sc_fdma16/, 与 K=4 隔离。

注意: 复用主模型的 cfg, 但额外用 cfg.SWIN_* 那一组超参
      SWIN_CODEWORD_DIM 必须等于 DIM_FDMA_PER_USER * 2 才公平 (16*2=32)
"""
...
def main():
    ...
    pbar = tqdm(range(start_step, cfg.TOTAL_STEPS + 1),
                desc="Swin-Baseline-8UE", dynamic_ncols=True)
